<!DOCTYPE html>
<html lang="{{ $locale }}" class="h-full bg-slate-950 text-slate-100">
<head>
    <meta charset="utf-8">
    ...
    @if(!empty($tenant->settings['adsense_client_id']))
    <script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client={{ $tenant->settings['adsense_client_id'] }}" crossorigin="anonymous"></script>
    @endif

    <!-- Dynamic Custom CSS Injection -->
    @if(!empty($tenant->settings['custom_css']))
    <style>
    ...
    <!-- Header Injection -->
    @if(!empty($tenant->settings['header_injection']))
    {!! $tenant->settings['header_injection'] !!}
    @endif
</head>
<body class="min-h-full flex flex-col justify-between relative bg-slate-950 text-slate-100 overflow-x-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
            <a href="{{ route('tenant.home', ['locale' => $locale]) }}" class="flex items-center space-x-3">
                @if(!empty($tenant->settings['logo']))
                    <img src="{{ $tenant->settings['logo'] }}" alt="{{ $tenant->name }}" class="h-8 w-auto">
                @else
            ...
            <div class="flex items-center space-x-4">
                @if(isset($tenant->supported_locales) && count($tenant->supported_locales) > 1)
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false" 
                            ...
                            @foreach($tenant->supported_locales as $lang)
    ...
    @if(!empty($tenant->settings['adsense_client_id']) && !empty($tenant->settings['adsense_top_slot']))
        <div class="max-w-7xl mx-auto px-6 py-4 flex justify-center">
            ...
            <ins class="adsbygoogle"
                 ...
                 data-ad-slot="{{ $tenant->settings['adsense_top_slot'] }}"></ins>
            <script>
    ...
    <!-- Footer Injection -->
    @if(!empty($tenant->settings['footer_injection']))
    {!! $tenant->settings['footer_injection'] !!}
    @endif
</body>
</html>
